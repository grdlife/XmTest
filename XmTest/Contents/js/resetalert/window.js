var win = new function () {
    ...
    function messageBox(html, title, message,height,width) {
        win.close();
        var jq = $(html);
    ...
    // 提示框
    this.ialert = function (title, message, closed) {
        this._close = function () {
            this.close();
            if ($.isFunction(closed)) closed();
        };
    ...
    this.alert = function (message) {
        ...
        var html = '<div class="win"><div class="mask-layer"></div><div class="window-panel"><iframe class="title-panel" frameborder="0" marginheight="0" marginwidth="0" scrolling="no"></iframe><div class="title"><h3></h3></div><a href="javascript:void(0)" onclick="win._close();" class="close-btn" title="关闭">×</a><div class="body-panel"><p class="content"></p></div></div></div>';
        messageBox(html, '系统提示', message, 100, 300);
    }
    function isNullOrEmpty(msg)
    {
        if (msg == '' || msg == null || typeof(msg) == "undefined" || msg.length == 0)//$.trim(msg)==''
            return true;
        else
            return false;
    }
    function isNotNullOrEmpty(msg) {
